...
train_index <- createDataPartition(total_data$profit, p=0.8, list=F)
...
ggplot(bst_mod$evaluation_log, aes(x = iter, y = test_error_mean)) + 
...
pd1 <- cbind.data.frame(bst_mod_eta3$evaluation_log[,c("iter", "test_error_mean")], rep(0.3, nrow(bst_mod_eta3$evaluation_log)))
names(pd1)[3] <- "eta"

# Extract results for model with eta = 0.1
pd2 <- cbind.data.frame(bst_mod_eta1$evaluation_log[,c("iter", "test_error_mean")], rep(0.1, nrow(bst_mod_eta1$evaluation_log)))
names(pd2)[3] <- "eta"

# Extract results for model with eta = 0.05
pd3 <- cbind.data.frame(bst_mod_eta05$evaluation_log[,c("iter", "test_error_mean")], rep(0.05, nrow(bst_mod_eta05$evaluation_log)))
names(pd3)[3] <- "eta"

# Extract results for model with eta = 0.01
pd4 <- cbind.data.frame(bst_mod_eta01$evaluation_log[,c("iter", "test_error_mean")], rep(0.01, nrow(bst_mod_eta01$evaluation_log)))
names(pd4)[3] <- "eta"

# Extract results for model with eta = 0.005
pd5 <- cbind.data.frame(bst_mod_eta005$evaluation_log[,c("iter", "test_error_mean")], rep(0.005, nrow(bst_mod_eta005$evaluation_log)))
names(pd5)[3] <- "eta"
...
```{r eta Plotting}
ggplot(plot_data, aes(x = iter, y = test_error_mean, color = eta))+
...
max_depth <- c(3, 4, 5, 6) #4 5
min_child_wt <- c(1, 3, 5) #1
...
ggplot(res_db, aes(y = max_depth, x = min_child_wt, fill = auc)) + # set aesthetics
...
ggplot(res_db, aes(y = max_depth, x = min_child_wt, fill = error)) + # set aesthetics
  geom_tile() + # Use geom_tile for heatmap
  theme_bw() + # Set theme
...
for(i in 1:length(gamma_vals)){
  ...
  bst_tune <- xgb.cv(data = dtrain, 
              ...
              eta = 0.3, 
              max.depth = 5, #From analysis above
...
names(res_db)[2:3] <- c("auc", "error") 

ggplot(res_db, aes(x = gamma_vals, y = auc)) + 
...
ggplot(res_db, aes(x = gamma_vals, y = error)) + 
  geom_point(color = "darkgreen", size = 5) + 
  theme_bw() + 
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
...
bst_tuned <- xgboost(data = dtrain, 
              ...
              max.depth = 5, 
              min_child_weight = 1, 
              gamma = 0.1, #When combined with all other parameters 0.15 gives better results than 0.1 
              subsample = 1,
              colsample_bytree = 0.9, 
...
bst_tuned_pred_class <- rep(0, length(bst_tuned_preds))
bst_tuned_pred_class[bst_tuned_preds >= 0.5] <- 1

t <- table(bst_tuned_pred_class, test_db$profit) 
